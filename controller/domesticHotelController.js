...
export const domesticHotelCreate = asyncHandel(async (req, res) => {
    try {
        ...
        let { township_id, title, price, rating, color, tags, description } = req.body;
        ...
        const uploadFolder = path.join(process.cwd(), "images", "domestic");
        ...
        const [data] = await db.query(
            `
               ...
               (township_id,title,price,rating,color,tags,description)
               VALUES(?,?,?,?,?,?,?)
            `,
            [township_id, title, price, rating, color, JSON.stringify(tags), description]
        )
...
export const domesticHotelUpdate = asyncHandel(async (req, res) => {
    try {
        ...
        const { id } = req.params;
        let { township_id, title, price, color, tags, description } = req.body;
        const [hotel] = await db.query("SELECT * FROM domestic_hotels WHERE id = ?", [id]);
        if (hotel.length === 0) {
        ...
        if (req.file) {
            if (hotel[0].image) {
                const oldPath = path.join(process.cwd(), hotel[0].image)
        ...
        const [data] = await db.query(
            ...
            WHERE id = ?
            `,
            [township_id, title, price, rating, color, JSON.stringify(tags), description, id]
        );
        return res.status(200).json({
            success: true,
...
export const domesticHotelDelete = asyncHandel(async (req, res) => {
    try {
        ...
        const [data] = await db.query(
            `
             DELETE FROM domestic_hotel WHERE id = ?
            `,
            [id]
        )
        return res.status(200).json({
            success: true,
            message: "Hotele Delete Success"
        });
